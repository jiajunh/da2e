...
class TransformerAgent(nn.Module):
    def __init__(self, envs, args):
        ...
        with torch.no_grad():
            ...
            flat_dim_v  = self.cnn_v(dummy).shape[-1]
        ...
        self.state_proj_v = nn.Sequential(nn.Linear(flat_dim_v, self.dim_v), nn.ReLU())
        self.v = layer_init(nn.Linear(self.dim_v, 1), std=1.0)

    def embed_state(self, obs):
        z = self.cnn(obs)
        return self.state_proj(z)

    def embed_state_v(self, obs):
        z = self.cnn_v(obs)
        return self.state_proj_v(z)
    ...
    def evaluate_values_only_onebatch(self, flat_obs, flat_actions, flat_ts, splits):
        s_flat = self.embed_state_v(flat_obs)          # (M, dim)
        v_flat = self.v(s_flat).squeeze(-1)        # (M,)
        return v_flat
    ...
    def evaluate_actions(self, flat_obs, flat_actions, flat_ts, splits):
        ...
        s_v = self.embed_state_v(obs_b.reshape(B*Lmax, C, H, W)).reshape(B, Lmax, self.dim_v)
        val = self.v(s_v).squeeze(-1)  # (B, L)
...
if __name__ == "__main__":
    ...
    run_name = f"{args.exp_name}_seed_{args.seed}_layers_{args.transformer_layers}_dim_{args.transformer_dim}_heads_{args.num_heads}_dropout_{args.dropout}_cnn_dim_{args.cnn_feature_dim}_schedule_{args.lr_schedule}_lr_{args.learning_rate}"
    print(args)
