...
def parse_args():
    ...
    # transformer
    parser.add_argument("--piv_transformer_layers", type=int, default=1)
    parser.add_argument("--adv_transformer_layers", type=int, default=1)
    parser.add_argument("--piv_transformer_dim", type=int, default=128)
    parser.add_argument("--adv_transformer_dim", type=int, default=128)
    parser.add_argument("--piv_num_heads", type=int, default=2)
    parser.add_argument("--adv_num_heads", type=int, default=2)
...
class SharedPVNet(nn.Module):
    def __init__(self, envs, args):
        ...
        self.backbone = DTBackbone(
            obs_dim,
            n_actions,
            args.piv_transformer_dim,
            args.piv_transformer_layers,
            args.piv_num_heads,
            args,
        )
        self.policy_head = layer_init(nn.Linear(self.backbone.D, n_actions), std=0.01)
        self.value_head = layer_init(nn.Linear(self.backbone.D, 1), std=1.0)
    ...
    def act_kvcache_per_env(self, obs_t, ts_t, prev_action, is_new_ep, kvs_list):
        device = obs_t.device
        N = obs_t.shape[0]
        A = self.policy_head.out_features
        ...
        if cont_idx.numel() > 0:
            past_kvs, past_mask, _ = pack_kvs_padded(
            ...
            t_s = self.backbone.embed_timestep(
                ts_c.clamp(max=self.backbone.embed_timestep.num_embeddings - 1)
            )
            t_a = self.backbone.embed_timestep(
                ts_a.clamp(max=self.backbone.embed_timestep.num_embeddings - 1)
            )
        ...
        if new_idx.numel() > 0:
            ...
            t = self.backbone.embed_timestep(
                ts_n.clamp(max=self.backbone.embed_timestep.num_embeddings - 1)
            )
            s = s + t
    ...
    def forward_values_tokens_dt(self, flat_obs, flat_act, flat_ts, splits):
        ...
        x = x_full[:, :-1, :]                                                   # drop last action token
        ...
        v = self.value_head(h_s.reshape(B * Lmax, D)).squeeze(-1).reshape(B, Lmax)
    ...
    def evaluate_state_policy_value(self, data_obs, data_ts, data_actions, splits):
        """
        data_obs:     (M, C, H, W)
        data_ts:      (M,)
        data_actions: (M,)
        ...
        n_actions = self.policy_head.out_features
        D = self.backbone.D
        ...
        logits = self.policy_head(h_s_flat)
        values_flat = self.value_head(h_s_flat).squeeze(-1)
        ...
        probs_list, entropy_list, values_list = [], [], []
        for b, L in enumerate(splits):
        ...
        return new_dist, new_logprobs, entropy_out, new_values
...
class AdvNet(nn.Module):
    def __init__(self, envs, args):
        ...
        self.backbone = DTBackbone(
            obs_dim,
            n_actions,
            args.adv_transformer_dim,
            args.adv_transformer_layers,
            args.adv_num_heads,
            args,
        )
    ...
    def evaluate_advantage(self, data_obs, data_ts, data_actions, old_dist, splits):
        """
        data_obs:     (M, C, H, W)
        data_ts:      (M,)
        data_actions: (M,)
        ...
        adv_raw = self.advantage_net(h_s).reshape(B, Lmax, n_actions)
        ...
        adv = adv_raw - torch.sum(old_dist_batch * adv_raw, dim=-1, keepdim=True)

        adv_list = []
        for b, L in enumerate(splits):
            adv_list.append(adv[b, :L])
...
if __name__ == "__main__":
    ...
    run_name = f"{args.exp_name}_seed_{args.seed}_piv_layers_{args.piv_transformer_layers}_adv_layers_{args.adv_transformer_layers}_piv_dim_{args.piv_transformer_dim}_adv_dim_{args.adv_transformer_dim}_piv_heads_{args.piv_num_heads}_adv_heads_{args.adv_num_heads}_cnn_dim_{args.cnn_feature_dim}_schedule_{args.lr_schedule}_entropy_{args.ent_coef}_lr_{args.learning_rate}"
    print(args)
    if args.track:
        import wandb
    ...
    piv = SharedPVNet(envs, args).to(device)
    adv = AdvNet(envs, args).to(device)

    optimizer_piv = torch.optim.Adam(
        piv.parameters(), 
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        optimizer_piv.param_groups[0]["lr"] = lrnow
        optimizer_adv.param_groups[0]["lr"] = lrnow_vf
        ...
        for step in range(0, args.num_steps):
            ...
            with torch.no_grad():
                action, logprob, probs = piv.act_kvcache_per_env(
                    obs_t=next_obs,
        ...
        with torch.no_grad():
            flat_obs, flat_act, flat_ts, splits, flat_t, flat_env, is_boot, boot_pos = flatten_with_bootstrap_dt(
                ...
                pad_action_id=piv.pad_action_id,
            )


            flat_v = forward_values_tokens_dt_traj_minibatches(
                piv, flat_obs, flat_act, flat_ts, splits,
        ...
        for epoch in range(args.update_epochs):
            for data in traj_buffer.get_trajs(batch_size=args.batch_size):
                ...
                new_dist, new_logprobs, entropy, new_values_flat = piv.evaluate_state_policy_value(
                    data_obs, data_ts, data_actions, splits=data_splits
                )

                new_advantages = adv.evaluate_advantage(
                    data_obs, data_ts, data_actions, old_dist=old_dist, splits=data_splits
                )

                # ----- split values by traj -----
                new_values = new_values_flat.flatten().split(data_splits)

                # ----- value loss for piv: detach A -----
                adv_taken_detached = new_advantages.detach().gather(
                    dim=1, index=data_actions.long().view(-1, 1)
                ).flatten()

                deltas_for_piv = (data_rewards - adv_taken_detached).split(data_splits)

                v_loss = compute_value_loss(
                    deltas_for_piv, new_values, data_last_values, discount_matrix, discount_vector
                )

                with torch.no_grad():
                    y_true, y_pred = dae_targets_and_preds(
                        deltas=deltas_for_piv,
                        values=new_values,
                ...
                # ----- PPO policy loss uses detached advantages -----
                eps = 1e-8
                old_logprobs = torch.log(old_dist.clamp(min=eps))
                kl_loss = (old_dist * (old_logprobs - new_logprobs)).sum(dim=1).mean()

                pg_advantages = new_advantages.detach().clone()
                if args.norm_adv:
                    pg_advantages = normalize_advantage(pg_advantages, old_dist)
                ...
                piv_loss_total = policy_loss_total + args.vf_coef * v_loss

                # ----- advantage loss for adv: detach V -----
                adv_taken = new_advantages.gather(
                    dim=1, index=data_actions.long().view(-1, 1)
                ).flatten()
                deltas_for_adv = (data_rewards - adv_taken).split(data_splits)

                detached_values = [v.detach() for v in new_values]
                adv_loss = compute_value_loss(
                    deltas_for_adv, detached_values, data_last_values, discount_matrix, discount_vector
                )
                adv_loss_total = args.vf_coef * adv_loss

                # ----- update piv -----
                optimizer_piv.zero_grad()
                piv_loss_total.backward()
                policy_grad_norm = nn.utils.clip_grad_norm_(piv.parameters(), args.max_grad_norm)
                optimizer_piv.step()

                # ----- update adv -----
                optimizer_adv.zero_grad()
                adv_loss_total.backward()
                value_grad_norm = nn.utils.clip_grad_norm_(adv.parameters(), args.max_grad_norm_vf)
                optimizer_adv.step()
        ...
        if args.track:
            wandb.log(
                {
                    "losses/adv_loss": adv_loss_total.item(),
                    "losses/piv_loss": piv_loss_total.item(),
