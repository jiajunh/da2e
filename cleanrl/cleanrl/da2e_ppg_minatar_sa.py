...
def parse_args():
    ...
    parser.add_argument("--learning_rate_aux", type=float, default=2.5e-4)
    ...
    parser.add_argument("--beta_clone", type=float, default=0.1)
    parser.add_argument("--aux_update_epochs", type=int, default=1)
    parser.add_argument("--aux_freq", type=int, default=5)
    parser.add_argument("--v_backbone", action="store_true")
    parser.add_argument("--adv_backbone", action="store_true")
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        self.v_backbone = args.v_backbone
        self.adv_backbone = args.adv_backbone
        ...
        self.aux_value_net = layer_init(nn.Linear(args.transformer_dim, 1), std=1.0)
    ...
    def forward_values_tokens_dt(self, flat_obs, flat_act, flat_ts, splits):
        ...
        x_full = torch.stack([s, a], dim=2).reshape(B, 2 * Lmax, D)  # (B, 2*Lmax, D)
        ...
        v = self.value_net(h_s.reshape(B * Lmax, D)).squeeze(-1).reshape(B, Lmax)  # (B, Lmax)
    ...
    def evaluate_state(self, data_obs, data_ts, data_actions, old_dist, splits):
        ...
        token_mask_full = torch.stack([pad_mask, pad_mask], dim=2).reshape(B, 2 * Lmax)  # (B, 3*Lmax)
        ...
        # value/adv heads do NOT send grads into backbone
        if self.adv_backbone:
            advantages_raw = self.advantage_net(h_s_flat)
        else:
            advantages_raw = self.advantage_net(h_det)

        if self.v_backbone:
            values_flat = self.value_net(h_s_flat).squeeze(-1)
        else:
            values_flat = self.value_net(h_det).squeeze(-1)
    ...
    def get_pi_value_and_aux_value(self, data_obs, data_ts, data_actions, old_dist, splits):
        device = data_obs.device
        B = len(splits)  # number of trajectories in batch
        Lmax = max(splits)  # maximum trajectory length
        C, H, W = data_obs.shape[1:]
        ...
        aux_values_flat = self.aux_value_net(h_s_flat).squeeze(-1)
        
        # Reshape back to batched form
        probs = probs_flat.reshape(B, Lmax, n_actions)
        aux_values = aux_values_flat.reshape(B, Lmax)
        ...
        cursor = 0
        for b, L in enumerate(splits):
            aux_values_list.append(aux_values[b, :L])
            probs_list.append(probs[b, :L])
            
        aux_values_out = torch.cat(aux_values_list, dim=0)
        new_dist = torch.cat(probs_list, dim=0)
        
        return new_dist, aux_values_out
...
class TrajectoryBuffer:
    def build_dae_traj_buffer(
        ...
        self.dae_targets = torch.zeros((len(self.observations),), device=device, dtype=torch.float32)
    

    @torch.no_grad()
    def compute_dae_targets_in_buffer_order(self, agent, discount_matrix, discount_vector):
        for traj_idx in range(len(self.start_indices)):
            start, end = self.start_indices[traj_idx], self.end_indices[traj_idx]
            L = end - start

            data_obs = self.observations[start:end]
            data_actions = self.actions[start:end]
            data_rewards = self.rewards[start:end]
            old_dist = self.old_probs[start:end]
            data_ts = self.timesteps[start:end]
            data_last = [self.last_values[traj_idx]]   # list length 1
            splits = [L]

            # teacher eval (same as PPO phase): value + centered advantage
            values_out, adv_out, _, _, _ = agent.evaluate_state(
                data_obs, data_ts, data_actions, old_dist=old_dist, splits=splits
            )

            deltas = (data_rewards - adv_out.gather(1, data_actions.long().view(-1, 1)).flatten()).split(splits)
            values_split = values_out.flatten().split(splits)

            y_true, _ = dae_targets_and_preds(
                deltas=deltas,
                values=values_split,
                lasts=data_last,
            ...
            self.dae_targets[start:end] = y_true
    ...
    def _get_traj_samples(self, indices):
        _obs, _act, _rew, _prob, _lpol, _val, _last, _timesteps, splits, _dae_targets = (
            [],
            [],
            [],
            [],
        ...
        for idx in indices:
            ...
            _dae_targets.append(self.dae_targets[start:end])
...
def lr_multiplier(step_idx, total_steps, args):
    ...
    lr_ratio = args.min_lr_ratio + (1.0 - args.min_lr_ratio) * decay_ratio
    return lr_ratio
...
if __name__ == "__main__":
    ...
    run_name = f"{args.exp_name}_seed_{args.seed}_layers_{args.transformer_layers}_dim_{args.transformer_dim}_heads_{args.num_heads}_dropout_{args.dropout}_cnn_dim_{args.cnn_feature_dim}_schedule_{args.lr_schedule}_entropy_{args.ent_coef}"
    print(args)
    if args.track:
        import wandb
    ...
    backbone_params = list(agent.embed_state.parameters()) + list(agent.embed_action.parameters()) + list(agent.embed_timestep.parameters())  + list(agent.transformer.parameters())
    policy_params = backbone_params + list(agent.action_net.parameters())
    vadv_params = list(agent.value_net.parameters()) + list(agent.advantage_net.parameters())

    optimizer = torch.optim.Adam(
        [
            {"params": backbone_params + list(agent.action_net.parameters()),
            "lr": args.learning_rate},
            {"params": list(agent.value_net.parameters()) + list(agent.advantage_net.parameters()),
            "lr": args.learning_rate_vf},
        ],
        eps=1e-8, weight_decay=args.weight_decay, betas=(args.beta1, args.beta2)
    )

    optimizer_aux = torch.optim.Adam(
        [
            {"params": backbone_params + list(agent.aux_value_net.parameters()), 
            "lr": args.learning_rate_aux},
        ],
        eps=1e-8, weight_decay=args.weight_decay, betas=(args.beta1, args.beta2),
    )
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        lr_v_now = lr_mul * args.learning_rate_vf
        lr_aux = lr_mul * args.learning_rate_aux

        optimizer.param_groups[0]["lr"] = lrnow
        optimizer.param_groups[1]["lr"] = lr_v_now

        optimizer_aux.param_groups[0]["lr"] = lr_aux
        ...
        with torch.no_grad():
            ...
            flat_v = forward_values_tokens_dt_traj_minibatches(
                agent, flat_obs, flat_act, flat_ts, splits,
        ...
        traj_buffer.compute_dae_targets_in_buffer_order(agent, discount_matrix, discount_vector)
        # break
        ...
        for epoch in range(args.update_epochs):
            for data in traj_buffer.get_trajs(batch_size=args.batch_size):
                ...
                new_advantages = new_advantages.detach()
                if args.norm_adv:
        ...
        if args.aux_freq > 0 and iteration % args.aux_freq == 0:
            for epoch in range(args.aux_update_epochs): 
                for data in traj_buffer.get_trajs(batch_size=args.batch_size):
                    (
                        data_obs, 
                        data_actions, 
                    ...
                    new_dist, new_aux_values = agent.get_pi_value_and_aux_value(
                        data_obs,
                        data_ts,
                        data_actions,
                        old_dist=old_dist,
                    ...
                    aux_value_loss = 0.5 * ((new_aux_values - data_targets.detach())**2).mean()
                    
                    eps = 1e-8
                    old_logp = torch.log(old_dist.clamp(min=eps))
                    new_logp = torch.log(new_dist.clamp(min=eps))
                    kl_loss = (old_dist * (old_logp - new_logp)).sum(dim=1).mean()

                    aux_loss = aux_value_loss + args.beta_clone * kl_loss

                    optimizer_aux.zero_grad()
                    aux_loss.backward()
                    aux_grad_norm = nn.utils.clip_grad_norm_(backbone_params + list(agent.aux_value_net.parameters()), args.max_grad_norm)
                    optimizer_aux.step()

            if args.track:
                wandb.log(
                    {
                        "losses/aux_value_loss": aux_value_loss.item(),
                        "losses/aux_grad_norm": aux_grad_norm.item(),
                        "losses/aux_kl_loss": kl_loss.item(),
                        "charts/learning_rate_aux": lr_aux,
                    },
                    step=global_step,
                )
