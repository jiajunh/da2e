...
class Agent(nn.Module):
    ...
    def encode_future_adv_from_tokens(self, s, a, splits):
        B, Lmax, D = s.shape
        device = s.device
        ...
        token_mask_rev = torch.zeros(B, x.shape[1], device=device, dtype=torch.bool)

        for b, L in enumerate(splits):
            T = 2 * L - 1
            x_rev[b, :T] = torch.flip(x[b, :T], dims=[0])
            token_mask_rev[b, :T] = True

        latent_rev, _, _ = self.transformer(x_rev, pad_mask=token_mask_rev)

        # state positions in reversed stream, order is [s_{L-1}, ..., s_0]
        h_rev_states = latent_rev[:, 0::2, :]   # (B, Lmax, D)

        # flip valid part back to original order and keep padded shape
        h_adv = torch.zeros(B, Lmax, D, device=device, dtype=latent_rev.dtype)
        for b, L in enumerate(splits):
            h_adv[b, :L] = torch.flip(h_rev_states[b, :L], dims=[0])
    ...
    def evaluate_state(self, data_obs, data_ts, data_actions, old_dist, splits, lmax):
        ...
        h_adv = self.encode_future_adv_from_tokens(s, a, splits)   # (B, Lmax, D)
        advantages_raw = self.advantage_net(h_adv)    
