...
def parse_args():
    ...
    parser.add_argument("--project_name", type=str, default="ppo_atari")
...
class Agent(nn.Module):
    ...
    def evaluate_actions(self, flat_obs, flat_act, flat_ts, splits, lmax):
        ...
        n_act = self.action_net.out_features
        ...
        ts_s = ts_b.clamp(max=self.embed_timestep.num_embeddings - 1)
        ts_a = (ts_b - 1).clamp(min=0, max=self.embed_timestep.num_embeddings - 1)

        s = self.embed_state(obs_b.reshape(B * Lmax, C, H, W)).reshape(B, Lmax, D)
        s = s + self.embed_timestep(ts_s)
        a = self.embed_action(prev_act_b) + self.embed_timestep(ts_a)

        x_full = torch.stack([a, s], dim=2).reshape(B, 2 * Lmax, D)
        x = x_full[:, :-1, :]
        ...
        v_flat = self.value_net(h_s_flat).squeeze(-1)

        act_padded = act_b.reshape(B * Lmax)
        logp_flat = dist.log_prob(act_padded.clamp(max=n_act - 1))
        ent_flat = dist.entropy()

        logp_out, ent_out, v_out = [], [], []
        cursor = 0
        for b, L in enumerate(splits):
            base = b * Lmax
            logp_out.append(logp_flat[base:base + L])
            ent_out.append(ent_flat[base:base + L])
            v_out.append(v_flat[base:base + L])
        return (
            torch.cat(logp_out, 0),
            torch.cat(ent_out, 0),
            torch.cat(v_out, 0),
        )
    ...
    def get_value(self, obs, ts):
        obs  = obs.float() / 255.0
        ts_c = ts.clamp(max=self.embed_timestep.num_embeddings - 1)
        s = self.embed_state(obs) + self.embed_timestep(ts_c)
        latent, _ = self.transformer(s.unsqueeze(1), past_kvs=None)
        return self.value_net(latent[:, -1, :]).squeeze(-1)
...
def _values_traj_minibatches(agent, flat_obs, flat_action, flat_ts, splits,
    ...
    for i in range(0, n_traj, traj_minibatch_size):
        ...
        _, _, v = agent.evaluate_actions(
            flat_obs=flat_obs[mb_start:mb_end],
            flat_act=flat_action[mb_start:mb_end],
...
if __name__ == "__main__":
    ...
    run_name = (
        ...
        f"cnn_dim={args.cnn_feature_dim}, ent_coef={args.ent_coef}"
    )
    ...
    for iteration in pbar:
        ...
        if iteration % args.log_freq == 0:
            ...
            pbar.set_description(
                f"speed: {speed:5.0f} sps | "
                f"ret: {avg_ret:6.2f} | "
                f"v_loss: {update_stats['v_loss']:.3f} | "
                f"pg_loss: {update_stats['pg_loss']:.3f} | "
                f"lr: {lr:.2e}"
            )
