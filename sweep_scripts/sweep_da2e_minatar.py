...
# -----------------------------------------------------------------------------
# Sweep generator for cleanrl/da2e_minatar_sa.py
# -----------------------------------------------------------------------------
...
num_envs = [1024, 2048, 256, 1024]
num_steps = [64, 32, 256, 32]
batch_size = [1024]
update_epochs = [5]
clip_coef = [0.1]
ent_coef = [0.5]
...
env_pairs = list(zip(num_envs, num_steps))
...
CMD_TEMPLATE = (
    ...
    "--wandb_project_name sweep_da2e_{proj_name} "
...
for (
    ...
    num_envs_, num_steps_ = env_pairs
...
def flush(count: int):
    ...
    script = os.path.join(out_dir, f"sweep_da2e_minatari_{batch_idx}.slurm")
    
    with open(script, "w") as f:
        f.write("#!/bin/bash\n")
        f.write("#SBATCH --job-name=sweep_da2e_minatari\n")
