...
learning_rate = [5.0e-4, 5.0e-4]
learning_rate_vf = [2.5e-4, 5.0e-4]
batch_size = [1024]
update_epochs = [5]
clip_coef = [0.1]
...
vf_coef = [1.0]
pi_transformer_layers = [1, 2]
pi_transformer_dim = [64, 128, 256]
pi_num_heads = [2, 4]
v_dim = [128, 256, 512]
cnn_feature_dim = [128]
warmup_frac = [0.10]
...
# ───────── COMMAND TEMPLATE ─────────
CMD_TEMPLATE = (
    "python cleanrl/da2e_separate_minatar_mlp_vadv_sa.py "
    "--seed {seed} "
    "--env_id {env_id} "
    "--total_timesteps {total_timesteps} "
    "--wandb_project_name sweep_da2e_mlp_vadv_separate_{proj_name} "
...
def flush(count: int):
    ...
    script = os.path.join(out_dir, f"sweep_da2e_separate_mlp_vadv_{batch_idx}.slurm")
    with open(script, "w") as f:
        f.write("#!/bin/bash\n")
        f.write("#SBATCH --job-name=sweep_da2e_separate_mlp_vadv_minatari\n")
