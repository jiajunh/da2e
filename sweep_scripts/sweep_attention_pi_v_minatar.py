...
# -----------------------------------------------------------------------------
# Sweep generator for cleanrl/attention_ppo_minatar_sa.py
# -----------------------------------------------------------------------------
...
parser.add_argument("--memory", type=int, default=24, help="Memory in GiB")
...
CMD_TEMPLATE = (
    ...
    "--wandb_project_name sweep_attention_pi_v_{proj_name} "
...
def flush(count: int):
    ...
    script = os.path.join(out_dir, f"sweep_attention_pi_v_{batch_idx}.slurm")
    with open(script, "w") as f:
        f.write("#!/bin/bash\n")
        f.write("#SBATCH --job-name=sweep_attention_pi_v_minatari\n")
