...
# -----------------------------------------------------------------------------
# Sweep generator for cleanrl/attention_pi_minatar_sa.py
#       - ppo, no advantage
#       - pi with transformer, v independent cnn
...
parser.add_argument("--nhrs", type=int, default=2, help="Job runtime in hours")
parser.add_argument("--mins", type=int, default=00, help="Job runtime in minutes")
...
parser.add_argument("--parallel", type=int, default=4, help="parallel_per_task")
...
learning_rate = [2.0e-5, 1.0e-4, 5.0e-4]
...
# paired parameters
dim_pairs = list(zip(transformer_dim, value_dim))
...
CMD_TEMPLATE = (
    ...
    "--wandb_project_name sweep_attention_pi_{proj_name} "
...
for (
    ...
    transformer_dim_, value_dim_ = dim_pairs
    exp_name = (
    ...
    cmd = CMD_TEMPLATE.format(
        ...
        kl_coef=kl_coef,
        transformer_layers=transformer_layers,
        transformer_dim=transformer_dim_,
        num_heads=num_heads,
        dropout=dropout,
        cnn_feature_dim=cnn_feature_dim,
...
def flush(count: int):
    ...
    script = os.path.join(out_dir, f"sweep_attention_pi_{batch_idx}.slurm")
    with open(script, "w") as f:
        f.write("#!/bin/bash\n")
        f.write("#SBATCH --job-name=sweep_attention_pi_minatari\n")
