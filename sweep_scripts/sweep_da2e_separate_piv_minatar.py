...
parser.add_argument("--mins", type=int, default=30, help="Job runtime in minutes")
parser.add_argument("--cpus", type=int, default=3, help="number of cpus")
parser.add_argument("--gpu", type=int, default=1, help="GPUs per task")
parser.add_argument("--memory", type=int, default=20, help="Memory in GiB")
parser.add_argument("--parallel", type=int, default=3, help="parallel_per_task")
...
vf_coef = [1.0]
piv_transformer_layers = [1, 1, 2, 2] 
adv_transformer_layers = [1, 2, 2, 1]
piv_transformer_dim = [64, 64, 128, 128]
adv_transformer_dim = [64, 128, 64, 128]
piv_num_heads = [2, 2, 4, 4]
adv_num_heads = [2, 4, 4, 2]
cnn_feature_dim = [128]
warmup_frac = [0.10]
min_lr_ratio = [0.1]
beta1 = [0.9]
...
layers_pairs = list(zip(piv_transformer_layers, adv_transformer_layers))
dim_pairs = list(zip(piv_transformer_dim, adv_transformer_dim))
head_pairs = list(zip(piv_num_heads, adv_num_heads))
...
CMD_TEMPLATE = (
    ...
    "--wandb_project_name sweep_da2e_separate_piv_{proj_name} "
    ...
    "--vf_coef {vf_coef} "
    "--kl_coef {kl_coef} "
    "--piv_transformer_layers {piv_transformer_layers} "
    "--adv_transformer_layers {adv_transformer_layers} "
    "--piv_transformer_dim {piv_transformer_dim} "
    "--adv_transformer_dim {adv_transformer_dim} "
    "--piv_num_heads {piv_num_heads} "
    "--adv_num_heads {adv_num_heads} "
    "--cuda "
    "--dropout {dropout} "
...
for (
    ...
    piv_transformer_layers_, adv_transformer_layers_ = layers_pairs
    piv_transformer_dim_, adv_transformer_dim_ = dim_pairs
    piv_num_heads_, adv_num_heads_ = head_pairs
    ...
    cmd = CMD_TEMPLATE.format(
        ...
        vf_coef=vf_coef,
        kl_coef=kl_coef,
        piv_transformer_layers=piv_transformer_layers_,
        adv_transformer_layers=adv_transformer_layers_,
        piv_transformer_dim=piv_transformer_dim_,
        adv_transformer_dim=adv_transformer_dim_,
        piv_num_heads=piv_num_heads_,
        adv_num_heads=adv_num_heads_,
        dropout=dropout,
        cnn_feature_dim=cnn_feature_dim,
        weight_decay=weight_decay,
        lr_schedule=lr_schedule,
...
def flush(count: int):
    ...
    script = os.path.join(out_dir, f"sweep_da2e_separate_piv_{batch_idx}.slurm")
    with open(script, "w") as f:
        f.write("#!/bin/bash\n")
        f.write("#SBATCH --job-name=sweep_da2e_separate_piv_minatari\n")
